...
- [x] Define content shapes for homepage hero, quick start cards, FAQ items, resource previews, templates, and troubleshooting entries.
- [x] Store bilingual content as predictable `en` / `zh` values rather than duplicating large JSX blocks in route files.
- [x] Add the approved `Quick Start` short-form UI copy to `content/site.ts`.
- [x] Add the revised `Learn` resource-hub content model to `content/learn.ts`.
- [x] Add starter-library and symptom-first support content to the templates and troubleshoot modules.
- [x] Run `npm run build` after content modules are wired into the pages.
...
- [x] Confirm one consistent bilingual pattern for short strings, summaries, bullets, and CTA labels.
- [x] Update helper types or rendering utilities only as needed so route files can stay data-driven.
- [x] Avoid embedding long fragments of JSX in content modules unless the component API truly requires it.
- [x] Run `npm run build` and confirm English and Chinese content still render correctly.
...
- [x] Create a `PageHero` component that supports kicker, title, summary, CTA row, and optional side content.
- [x] Create a `SectionHeading` component for repeated title plus supporting copy layouts.
- [x] Replace duplicated hero and section-intro markup across route files with the new components.
- [x] Keep differences configurable through props instead of branching component logic by page.
- [x] Run `npm run build` and verify the shared components compile cleanly.
...
- [x] Build a dedicated `QuickStartCard` for title, summary, bullets, and CTA.
- [x] Extract repeated template card markup into `TemplateCard`.
- [x] Extract repeated troubleshooting issue or check markup into `TroubleshootCard` and `ChecklistList`.
- [x] Keep components generic enough for later detail-page expansion, but do not build unused abstraction.
- [x] Run `npm run build` and verify no regressions in any route file using these cards.
...
- [x] Add a visually distinct `Quick Start` button in the header that links to `/#quick-start`.
- [x] Keep `Learn`, `Templates`, and `Troubleshoot` as the normal top-level navigation items.
- [x] Ensure the new CTA works from every page and remains understandable in both language modes.
- [x] Audit spacing, focus states, and responsive behavior so the header does not collapse awkwardly on small screens.
...
- [x] Remove the old onboarding-style framing that repeats homepage responsibilities.
- [x] Rewrite the page intro so it explains `Learn` as a deeper resource hub.
- [x] Organize content into topic or collection groups such as Foundations, Setup, Skills and Workflow Design, Use Cases, Chinese Ecosystem, and Advanced Troubleshooting.
- [x] Preserve clear exits toward Templates and Troubleshoot where they help the user continue.
...
- [x] Reframe the page around starter workflows and scenario-based discovery.
- [x] Remove fake advanced filters or search controls that imply unsupported behavior.
- [x] Keep metadata useful and scannable: difficulty, time, risk, tags, and channels where relevant.
- [x] Ensure at least one featured starter template aligns with the homepage Quick Start recommendation.
...
- [x] Tighten the page around symptom-first checks for first-run problems.
- [x] Keep quick-check commands visible and easy to scan.
- [x] Add routing cues back to homepage Quick Start, Learn, or Templates where appropriate.
- [x] Remove UI affordances that imply features that do not exist yet.
...
- [x] Improve spacing and card stacking so the Quick Start section remains readable on narrow screens.
- [x] Verify the language and theme controls are still clearly labeled and keyboard accessible.
- [ ] Run `npm run build`, then run `npm run dev` and manually review all four pages in both themes and at multiple widths.
- [x] Record any residual issues before handoff if they are out of scope for the MVP.

## Delivery Checklist

- [x] Homepage hero routes users into `#quick-start`
- [x] Header includes a highlighted `Quick Start` action without changing the primary nav hierarchy
- [x] Homepage includes the five-card bilingual Quick Start section
- [x] Homepage previews support, rather than compete with, onboarding
- [x] Learn reads as a resource hub rather than a second onboarding page
- [x] Templates supports starter workflow discovery
- [x] Troubleshoot supports first-run recovery
- [x] Metadata matches the revised information architecture
...
- `npm run dev` starts successfully in this environment, but HTTP access to the local dev server is blocked from parallel command sessions here, so the remaining unchecked items require a real browser-based manual review outside this sandbox.
